...
---
## 简介
这是一个基于 Qwen 模型的金融问答助手微调项目，采用 Unsloth + TRL 对 Qwen2.5-7B-Instruct 做 4-bit + LoRA 的监督微调（SFT），解决通用大模型在金融问答中“回答不稳定、专业性不足、格式不统一”的问题。仓库包含数据混合构建、训练、teacher-forcing 与生成式评估，以及推理/聊天示例，适合需要复现实验、构建金融垂直助手或学习 LLM 微调工作流的人。
---
## 目录
